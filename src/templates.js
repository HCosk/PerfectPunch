...
function renderSummaryBreakdown(summary) {
  const entries = Object.entries(summary || {}).sort((left, right) => Number(right[1]) - Number(left[1]));
  if (!entries.length) {
...
function renderSessionsTable(sessions, emptyMessage = "No sessions recorded yet.") {
  if (!sessions.length) {
            ...
            .map((session) => {
              return `
                <tr>
                  ...
                  <td>${formatCount(session.total_events)}</td>
                </tr>
              `;
            })
            .join("")}
    ...
    content: `
      ...
      <section class="content-grid">
        <article class="card">
          ...
          ${renderSessionsTable(sessions, "Record a session to start building your dashboard.")}
        </article>
        <article class="card">
          <div class="card-head">
            <h2>Punch mix</h2>
...
function renderHistoryPage({ user, sessions }) {
  return renderLayout({
    ...
    content: `
      ...
      <section class="card">
        <div class="card-head">
          <h2>All sessions</h2>
          <p>Newest first, with left and right arm labels carried through.</p>
        </div>
        ${renderSessionsTable(sessions, "You have not saved any sessions yet.")}
      </section>
    `
  });
}
                    ...
                    .map((event, index) => {
                      return `
                        <tr>
                          ...
                          <td><span class="status-pill ${event.status === "ok" ? "status-pill--ok" : "status-pill--warn"}">${escapeHtml(event.status)}</span></td>
                        </tr>
                      `;
                    })
                    .join("")}
  ...
  return renderLayout({
    ...
    content: `
      ...
      <section class="stat-grid">
        ...
        ${renderStatCard("Uncertain events", formatCount(session.uncertain_events), "Lower is cleaner")}
